...
@google_benchmark.option.use_real_time()
@google_benchmark.option.unit(google_benchmark.kMicrosecond)
@google_benchmark.option.arg_names(['segment_sample_length', 'three_fibers'])
@google_benchmark.option.args_product([[128, 256, 512, 1024, 2048, 4096], [True, False]])
def bench_jax_grad(state: google_benchmark.State):
  ...
  """
  ihc_style = 'two_cap_with_syn' if state.range(1) else 'two_cap'
  random_seed = 1
...
@google_benchmark.option.arg_names(
    ['jax_chunked_uncompiled', 'segment_sample_length', 'use_delay_buffer',  'three_fibers']
)
@google_benchmark.option.args_product(
    [[0, 1, 2], [220, 2205, 22050, 44100], [False, True], [ True, False]]
)
def bench_jax(state: google_benchmark.State):
  ...
  """
  # Inits JAX version
  ihc_style = 'two_cap_with_syn' if state.range(3) else 'two_cap'
  random_seed = 1
  params_jax = carfac_jax.CarfacDesignParameters()
