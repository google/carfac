...
def bench_jax_grad(state: google_benchmark.State):
  ...
  def loss_func(
    ...
    nap_output, _, _, _, _, _, _, _ = carfac_jax.run_segment(
...
def bench_jit_compile_time(state: google_benchmark.State):
  ...
  while state:
    ...
    n_samp += 1
    state.resume_timing()
    naps_jax, _, _, state_jax, _, _, _, _ = carfac_jax.run_segment_jit(
...
def bench_jax_in_slices(state: google_benchmark.State):
  ...
  for _, segment in enumerate(silence_slices):
    if segment.shape not in compiled_shapes:
      ...
      naps_jax, _, _, _, _, _, _, _ = carfac_jax.run_segment_jit(
  ...
  while state:
    ...
    state.resume_timing()
    for _, segment in enumerate(run_seg_slices):
      seg_naps, _, _, jax_loop_state, seg_bm, seg_ohc, seg_agc, seg_agc_memory = (
      ...
      ohc.append(seg_ohc)
      agc.append(seg_agc)
      agc_memory.append(seg_agc_memory)
...
def bench_jax(state: google_benchmark.State):
  ...
  short_silence = jnp.zeros(shape=(n_samp, n_ears))
  naps_jax, _, _, state_jax, _, _, _, _ = run_segment_function(
