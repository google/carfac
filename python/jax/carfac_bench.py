...
def bench_jit_compile_time(state: google_benchmark.State):
  ...
  while state:
    ...
    n_samp += 1
    state.resume_timing()
    naps_jax, _, state_jax, _, _, _, _, _ = carfac_jax.run_segment_jit(
...
def bench_jax_in_slices(state: google_benchmark.State):
  ...
  while state:
    ...
    state.resume_timing()
    for _, segment in enumerate(run_seg_slices):
      seg_naps, seg_naps_fibers, jax_loop_state, seg_bm, seg_receptor_pot, seg_ohc, seg_agc, seg_agc_actual = (
...
def bench_jax(state: google_benchmark.State):
  ...
  short_silence = jnp.zeros(shape=(n_samp, n_ears))
  naps_jax, _, state_jax, _, _, _, _, _ = run_segment_function(
