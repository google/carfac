...
class _TestCallable:
  ...
  def __init__(self, cell: carfac.CARFACCell):
    ...
    self.cell = cell
    self.layer = tf.function(
        tf.keras.layers.RNN(self.cell, return_sequences=True)
    )

  def __call__(self, audio: np.ndarray) -> np.ndarray:
    return self.layer(
        np.reshape(audio, (1, audio.shape[0], audio.shape[1], 1))
    ).numpy()[0]
...
class CARFACTest(tf.test.TestCase):
  ...
  def testConvolvers(self):
    ...
    for kernel_size in [3, 5]:
      ...
      data = tf.ones((3, 4, 92), dtype=tf.float64) * tf.range(
          92, dtype=tf.float64
      )
  ...
  def testRecurrenceExpanders(self):
    ...
    a_0 = tf.ones((3, 4), dtype=tf.float64) * tf.constant(
        0.51, dtype=tf.float64
    )
    f = 0.9 + (0.2 / 92) * tf.ones((3, 4, 92), dtype=tf.float64) * tf.range(
        92, dtype=tf.float64
    )
    g = 0.01 + (2.0 / 92) * tf.ones((3, 4, 92), dtype=tf.float64) * tf.range(
        92, dtype=tf.float64
    )
    ...
    carfac_testing.assert_matlab_compatibility(
        ...
        _TestCallable(
            carfac.CARFACCell(
                open_loop=True,
                num_ears=2,
                outputs=[carfac.CARFACOutput.BM, carfac.CARFACOutput.NAP],
  ...
  def testAGCDesignAtLowSampleRate(self):
    for i, spread in enumerate([0.0, 1.0, 1.4, 2.0, 2.8]):
      carfac_cell = carfac.CARFACCell(
          ...
          agc_params=carfac.AGCParams(
              agc1_scales0=tf.constant(1.0 * spread),
              agc2_scales0=tf.constant(1.65 * spread),
          ),
      )
      agc_coeffs = carfac_cell._design_agc_coeffs()
      if i == 0:
        self.assertTrue(np.all(agc_coeffs.agc_spatial_n_taps.numpy() == 3))
        self.assertTrue(np.all(agc_coeffs.agc_spatial_iterations.numpy() == 0))
      elif i == 1:
        self.assertTrue(np.all(agc_coeffs.agc_spatial_n_taps.numpy() == 5))
        self.assertTrue(np.all(agc_coeffs.agc_spatial_iterations.numpy() == 1))
      elif i == 2:
        self.assertTrue(np.all(agc_coeffs.agc_spatial_n_taps.numpy() == 5))
        self.assertTrue(np.all(agc_coeffs.agc_spatial_iterations.numpy() == 2))
      elif i == 3:
        self.assertTrue(np.all(agc_coeffs.agc_spatial_n_taps.numpy() == 5))
        self.assertTrue(np.all(agc_coeffs.agc_spatial_iterations.numpy() == 4))
      elif i == 4:
        self.assertTrue(np.all(agc_coeffs.agc_spatial_iterations.numpy() == -1))
  ...
  def testAGCDesignBehavesSensibly(self):
    for i, spread in enumerate([0.0, 1.0, 1.4, 2.0, 2.8, 3.5, 4.0]):
      ...
      if i == 0:
        ...
        self.assertTrue(np.all(agc_coeffs.agc_spatial_n_taps.numpy() == 5))
        self.assertTrue(np.all(agc_coeffs.agc_spatial_iterations.numpy() == 3))
      elif i == 5:
        self.assertTrue(np.all(agc_coeffs.agc_spatial_n_taps.numpy() == 5))
        self.assertTrue(np.all(agc_coeffs.agc_spatial_iterations.numpy() == 4))
      elif i == 6:
  ...
  def testModes(
    ...
    carfac_cell = carfac.CARFACCell(
        ihc_params=ihc_params, car_params=car_params, num_ears=3, dtype=dtype
    )
