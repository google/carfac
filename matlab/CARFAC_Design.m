...
AGC_coeffs = struct( ...
  ...
  'temporal_IIR', zeros(n_AGC_stages + 1, n_AGC_stages), ...
...
for stage = 1:n_AGC_stages
  ...
  if stage < n_AGC_stages
    AGC_coeffs.temporal_IIR(stage + 1, stage) = ...
      AGC_params.AGC_stage_gain * AGC_epsilon;
  end
  % Input gain
  AGC_coeffs.temporal_IIR(n_AGC_stages + 1, stage) = ...
    AGC_epsilon * detect_scale;
  % Leave the rest at 0.
